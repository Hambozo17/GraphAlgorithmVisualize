...
\begin{titlepage}
    ...
    {\Large\textbf{Nile University}}\\[2cm]
...
\begin{lstlisting}
function dfs(graph: Graph, startId: string, 
             detectCycles: boolean = true): AlgorithmResult {
  const steps: AlgorithmStep[] = [];
  const visited = new Set<string>();
  ...
  function dfsRecursive(nodeId: string, 
    ...
    for (const { node: neighbor, edge } of neighbors) {
      ...
      if (!visited.has(neighbor.id)) {
        ...
        steps.push({
          ...
          nodeId: neighbor.id,
          edgeId: edge.id,
          message: `Cycle detected at node ${neighbor.label}!`,
          visited: new Set(visited),
          stack: Array.from(recursionStack),
        });
        return true;
    ...
    steps.push({
      type: 'backtrack',
      nodeId,
      message: `Backtracking from node ${node?.label}`,
      visited: new Set(visited),
      stack: Array.from(recursionStack),
    });
...
\subsection{Bellman-Ford Implementation}

\begin{lstlisting}
function bellmanFord(graph: Graph, startId: string, 
                     endId?: string): AlgorithmResult {
  const steps: AlgorithmStep[] = [];
  const distances = new Map<string, number>();
  const parents = new Map<string, string | null>();
    ...
    for (const edge of graph.edges) {
      ...
      for (const { from, to } of edgesToCheck) {
        ...
        if (fromDist !== Infinity && fromDist + edge.weight < toDist) {
          distances.set(to, fromDist + edge.weight);
          parents.set(to, from);
          updated = true;
          ...
          steps.push({
            ...
            nodeId: to,
            edgeId: edge.id,
            message: `Relaxed edge: distance to ${getNodeById(graph, to)?.label} = ${fromDist + edge.weight}`,
            distances: new Map(distances),
          });
        }
      }
...
\begin{lstlisting}
function aStar(graph: Graph, startId: string, 
               endId: string): AlgorithmResult {
  const steps: AlgorithmStep[] = [];
  ...
  while (openSet.length > 0) {
    ...
    openSet.sort((a, b) => (fScore.get(a) || Infinity) - 
...
\begin{figure}[H]
    ...
    \caption{Graph Algorithm Visualizer main interface showing the interactive canvas, toolbar, and algorithm panel}
    \label{fig:interface}
\end{figure}
